...
int Socket::accept(InetAddress *peeraddr){
   ...
   /*
        // accept + 2次fcntl（非原子操作）
        connfd = accept(listenfd, ...);
        fcntl(connfd, F_SETFL, O_NONBLOCK);      // 设置非阻塞
        fcntl(connfd, F_SETFD, FD_CLOEXEC);      // 设置CLOEXEC

        // accept4 一步到位（原子操作）
        connfd = accept4(listenfd, ..., SOCK_NONBLOCK | SOCK_CLOEXEC);
   */
...
void Socket::setReuseAddr(bool on){
    ...
    /*
    不是任意"已被使用"都能绑定，主要针对 TIME_WAIT 状态：
        主动关闭连接的一方 → 进入 TIME_WAIT (2MSL)
        此时端口还未完全释放
        SO_REUSEADDR 允许新 socket 重用这个端口
    */
